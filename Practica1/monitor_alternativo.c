...
{
  MemoriaCompartida *shm = NULL;
  struct timespec espera_corta;
  espera_corta.tv_sec = 0;
  espera_corta.tv_nsec = 10000000; /*10 ms*/
  ...
  while ((*info = shm->cola).target == 0 ) {
      nanosleep(&espera_corta, NULL);
  }
  shm->cola.target = 0;
  munmap(shm, sizeof(MemoriaCompartida));
}
